...
| `test_set.pl` | region sets as folders, per-set ids, the ini selections and how they degrade, checked-is-a-view, and that **making a set neither opens it nor touches the startup pointer** - it used to do both as a side effect, which is how the pointer became a second copy of "what is open" that Close then cleared only one of |
...
| `test_coherence.pl` | **can this node be built, and if not, which of the four ways is it failing** - the predicate every act now shares. That the five states are five distinct values and not a boolean, that `''` is an answer rather than a gap to be filled in, that display asks less than build and the same tree can be fine for one and refused for the other, that a region with five inheriting subregions is ONE fault against the region while a subregion naming its own bad source answers for itself, that the remedy travels with the report, and that `regionSourceMap` no longer takes anything to pass a display source through - which is what makes the old bug unwritable rather than merely fixed. Entirely offline |
| `test_fill.pl` | the cache filler: which source each node resolves to, the zmax cap, the abort, and that an **incoherent tree is refused whole** rather than partly fetched - a fetch exists to put on disk what a build will read, so a fetch that succeeds where the build will refuse is an afternoon spent finding that out. Nearly offline - it plants the cache itself and asserts a 100% hit rate, which is what proves source resolution |
...
| `tool_preflight_demo.pl` | the **two preflight dialogs** over a fixture set of four regions - one that builds, one that chose no source, one naming a `.tsd` that is not installed, and one naming a display-only source - so all four coherence states are on screen at once. `cfg` shows dialog one, `pre` dialog two, `ok` dialog two with nothing wrong. It exists because what these two have to get right is what they SHOW, and no headless assertion can look at a dialog; the rules underneath are pinned by `test_coherence.pl`. It found three real defects on its first run - the faults rendered twice in two panels, remedy sentences long enough to be clipped by a panel that does not wrap, and a display-only region counted in the cost table directly under the line saying it was not counted |
